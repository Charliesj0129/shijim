...
def test_metrics_update(mock_worker):
    ...
    # If we are in an env with prometheus_client installed
    # (which we added to pyproject.toml but maybe not installed in venv yet?)
...
def test_throughput_monitor():
    ...
    # Since we can't easily read the gauge value without private API
    # or registry access in this setup,
...
def test_gap_detector():
    ...
    # First event
    event1 = MDTickEvent(
        ts_ns=1000, symbol="S1", asset_type="stock", exchange="E", price=1.0, size=1
    )
    detector.on_event(event1)

    # Second event, small gap (50ns)
    event2 = MDTickEvent(
        ts_ns=1050, symbol="S1", asset_type="stock", exchange="E", price=1.0, size=1
    )
    detector.on_event(event2)

    # Third event, large gap (200ns)
    event3 = MDTickEvent(
        ts_ns=1250, symbol="S1", asset_type="stock", exchange="E", price=1.0, size=1
    )
